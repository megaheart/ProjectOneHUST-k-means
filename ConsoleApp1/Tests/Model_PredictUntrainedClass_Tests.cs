...
namespace ConsoleApp1.Tests
{
    ...
    public delegate T TrainTestFunction<T>(TrainFunction<T> train, PredictFunction<T> predict) where T:class;
    public class Model_PredictUntrainedClass_Tests
    {
        ...
        public static void TestModel_AllData<T>(TrainFunction<T> train, PredictFunction<T> predict)
            where T : class
        {
            ...
            var excludedDataNames = new List<string>() { "Wine", "Statlog(Heart)", "Vertebral Column(2C)" };
            var testDatas = Data.datas.Where(x => !excludedDataNames.Contains(x.Name));

            foreach (var dataInfo in testDatas)
            {
                Debug.WriteLine(dataInfo.Name);
                ...
                foreach (var combination in Calculation.GenerateAllCombinations(pies, k))
                {
                    ...
                    Debug.WriteLine(
                        String.Format("Train Count: {0}, Trained Test Count: {1}, Untrained Test Count: {2}",
                        test_data.TrainData.Count, test_data.TestTrainedClassData.Count, test_data.TestUntrainedClassData.Count));
                    ...
                    Debug.WriteLine(test_data.TestTrainedClassLabels.Print());
                    Debug.WriteLine(trainedDataPredictions.Print());
                    Debug.WriteLine(test_data.TestUntrainedClassLabels.Print());
                    Debug.WriteLine(untrainedDataPredictions.Print());
        ...
        public static void TestModel<T>(TrainFunction<T> train, PredictFunction<T> predict)
            where T : class
        {
            TestModel_AllData(train, predict);
        }
